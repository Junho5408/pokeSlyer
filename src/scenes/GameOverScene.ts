...
export class GameOverScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height / 2 - 110, '💀', { fontFamily: '"Neo둥근모", monospace', fontSize: '75px' }).setOrigin(0.5)

    this.add.text(width / 2, height / 2 - 30, '게임 오버', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '55px', color: '#ff4444',
    }).setOrigin(0.5)
    ...
    stats.forEach(({ label, value }, i) => {
      ...
      this.add.text(width / 2 - 70, y, value, {
        fontFamily: '"Neo둥근모", monospace', fontSize: '21px', color: '#ffffff',
      }).setOrigin(0, 0.5)
    })

    // 다시 시작
    const retryBtn = this.add.text(width / 2 - 90, height - 90, '다시 시작', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '28px', color: '#aaffaa',
    ...
    const menuBtn = this.add.text(width / 2 + 90, height - 90, '메인 메뉴', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '28px', color: '#aaaaaa',
