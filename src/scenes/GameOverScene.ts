...
export class GameOverScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height / 2 - 110, '💀', { fontFamily: '"Neo둥근모", monospace', fontSize: '60px' }).setOrigin(0.5)

    this.add.text(width / 2, height / 2 - 30, '게임 오버', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '44px', color: '#ff4444', fontStyle: 'bold',
    }).setOrigin(0.5)
    ...
    stats.forEach(({ label, value }, i) => {
      ...
      this.add.text(width / 2 - 70, y, value, {
        fontFamily: '"Neo둥근모", monospace', fontSize: '17px', color: '#ffffff',
      }).setOrigin(0, 0.5)
    })

    // 다시 시작
    const retryBtn = this.add.text(width / 2 - 90, height - 90, '다시 시작', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '22px', color: '#aaffaa',
