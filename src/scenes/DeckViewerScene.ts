...
export class DeckViewerScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // 제목
    this.add
      .text(width / 2, 34, `덱 (${run.deck.length}장)`, {
    ...
    const groupMap = new Map<
      string,
      { defId: string; isUpgraded: boolean; count: number }
    >();
    ...
    entries.forEach(({ defId, isUpgraded, count }, i) => {
      ...
      c.add(
        this.add
          .text(-cardW / 2 + 10, -cardH / 2 + 10, String(def.cost), {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "10px",
            color: "#ffdd44",
          })
    ...
    this.add
      .text(width / 2, height - 22, "ESC 또는 배경 클릭으로 닫기", {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "11px",
        color: "#333333",
      })
