...
export class ShopScene extends Phaser.Scene {
  ...
  private buildShopView(run: RunState): void {
    ...
    const removeBtn = this.add
      .text(width / 2, 720, `카드 제거  ${REMOVAL_COST}G`, {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "22px",
    ...
    if (canRemove) {
      ...
      removeBtn.on("pointerover", () =>
        removeBtn.setStyle({ color: "#ffffff" }),
      );
      removeBtn.on("pointerout", () =>
        removeBtn.setStyle({ color: "#ffaaaa" }),
      );
    ...
    this.shopView.add(
      this.add
        .text(width / 2, 778, "덱에서 카드 1장을 영구 제거합니다", {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "14px",
          color: "#555555",
        })
  ...
  private buildShopCard(
    ...
    c.add(
      this.add
        .text(-cardW / 2 + 18, -cardH / 2 + 18, String(def.cost), {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "15px",
          color: "#ffdd44",
        })
    ...
    c.add(
      this.add
        .text(0, -cardH / 2 + 100, def.description, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "13px",
          color: "#dddddd",
  ...
  private buildRemoveView(run: RunState): void {
    ...
    this.removeView.add(
      this.add
        .text(
          width / 2,
          114,
          `비용: ${REMOVAL_COST}G  |  현재 덱: ${run.deck.length}장`,
          {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "13px",
            color: "#888888",
    ...
    uniqueCards.forEach(([defId, count], i) => {
      ...
      c.add(
        this.add
          .text(0, -cardH / 2 + 52, def.description, {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "9px",
            color: "#cccccc",
            wordWrap: { width: cardW - 8 },
      ...
      if (count > 1) {
        c.add(
          this.add
            .text(cardW / 2 - 4, -cardH / 2 + 4, `x${count}`, {
              fontFamily: '"Neo둥근모", monospace',
              fontSize: "10px",
              color: "#88ff88",
            })
