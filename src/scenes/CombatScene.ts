...
// ── 적 캐릭터 색상 테마 ────────────────────
const ENEMY_THEMES: Record<
  string,
  { body: number; head: number; stroke: number }
> = {
...
export class CombatScene extends Phaser.Scene {
  ...
  private enemyContainers: Map<string, Phaser.GameObjects.Container> =
    new Map();
  ...
  private buildStaticUI(): void {
    ...
    // 더미 카운트
    this.pileText = this.add.text(40, 1040, "", {
      fontFamily: '"Neo둥근모", monospace',
      fontSize: "21px",
      color: "#888888",
    });
    ...
    this.endTurnBtn.on("pointerover", () =>
      this.endTurnBtn.setStyle({ backgroundColor: "#4a6630" }),
    );
    this.endTurnBtn.on("pointerout", () =>
      this.endTurnBtn.setStyle({ backgroundColor: "#334422" }),
    );
  ...
  private refreshPlayerHud(): void {
    ...
    const bw = 300;
    this.playerHud.add(
      this.add.rectangle(0, 0, bw, 22, 0x552222).setOrigin(0, 0.5),
    );
    const hpFill = Math.max(0, r.hp / r.maxHp);
    this.playerHud.add(
      this.add.rectangle(0, 0, bw * hpFill, 22, 0xcc3333).setOrigin(0, 0.5),
    );
    this.playerHud.add(
      this.add
        .text(bw / 2, 0, `HP ${r.hp} / ${r.maxHp}`, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "18px",
          color: "#fff",
        })
    ...
    this.playerHud.add(
      this.add.text(0, 26, `🛡 방어도: ${r.block}`, {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "19px",
        color: "#88aaff",
      }),
    ...
    this.playerHud.add(
      this.add.text(0, 48, `⚡ 에너지: ${r.energy} / ${r.maxEnergy}`, {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "19px",
        color: r.energy > 0 ? "#ffdd44" : "#888888",
      }),
    );
  ...
  private buildEnemyView(
    ...
    const theme = ENEMY_THEMES[defId] ?? {
      body: 0x1c2a38,
      head: 0x2a3a4a,
      stroke: 0x4477aa,
    };
    ...
    if (intent) {
      ...
      c.add(
        this.add
          .text(0, headY - headR - 19, this.intentLabel(intent), {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "18px",
            color: "#ffffff",
          })
    ...
    namePlateG.fillRoundedRect(
      -50,
      headY - headR - (intent ? 54 : 26),
      100,
      20,
      4,
    );
    ...
    c.add(
      this.add
        .text(0, headY - headR - (intent ? 43 : 15), enemy.name, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "19px",
          color: isBoss ? "#ffaaaa" : "#ffffff",
        })
        .setOrigin(0.5),
    );
    ...
    hpG.fillRoundedRect(-bw / 2, barY, bw, 18, 3);
    const hpRatio = Math.max(0, enemy.resource.hp / enemy.resource.maxHp);
    if (hpRatio > 0) {
      hpG.fillStyle(
        hpRatio > 0.5 ? 0x44bb44 : hpRatio > 0.25 ? 0xbb8833 : 0xcc2222,
      );
      hpG.fillRoundedRect(-bw / 2, barY, bw * hpRatio, 18, 3);
    }
    ...
    c.add(
      this.add
        .text(0, barY + 7, `${enemy.resource.hp} / ${enemy.resource.maxHp}`, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "16px",
          color: "#ffffff",
          stroke: "#000000",
    ...
    if (enemy.resource.block > 0) {
      ...
      c.add(
        this.add
          .text(0, extraY + 9, `🛡 ${enemy.resource.block}`, {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "16px",
            color: "#aaccff",
          })
  ...
  private intentLabel(
    intent: NonNullable<ReturnType<CombatManager["getNextEnemyIntent"]>>,
  ): string {
    const v = intent.intent;
    switch (v.type) {
      case "ATTACK":
        return `⚔ ${v.value ?? 0}${v.times ? ` x${v.times}` : ""}`;
      case "DEFEND":
        return `🛡 방어`;
      case "BUFF":
        return `✨ 강화`;
  ...
  private buildCardView(
    ...
    const canAfford = this.manager.energyManager.canAfford(
      this.manager.state.player,
      def.cost,
    );
    const interactive =
      this.manager.state.isPlayerTurn && !this.manager.isProcessing;
  ...
  private refreshPileText(): void {
    ...
    this.pileText.setText(
      `드로우: ${p.draw.length}  버림: ${p.discard.length}  소멸: ${p.exhaust.length}`,
    );
    const active =
      this.manager.state.isPlayerTurn && !this.manager.isProcessing;
  ...
  private handleCardClick(inst: CardInstance): void {
    ...
    if (
      !this.manager.energyManager.canAfford(this.manager.state.player, def.cost)
    ) {
  ...
  private handleEnemyClick(enemyId: string): void {
    ...
    const inst = this.manager.state.piles.hand.find(
      (c) => c.instanceId === this.selectedCardId,
    );
    if (!inst) return;
    void this.doPlayCard(inst, enemyId);
  }

  private async doPlayCard(
    inst: CardInstance,
