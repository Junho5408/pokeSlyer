...
export class MainMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(width / 2, height / 2 - 100, 'pokeSlyer', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '65px', color: '#ffffff',
    }).setOrigin(0.5)

    this.add.text(width / 2, height / 2 - 46, '덱빌딩 로그라이크', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '20px', color: '#555577',
    }).setOrigin(0.5)
    ...
    const startBtn = this.add.text(width / 2, btnY, '새 게임 시작', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '32px', color: '#aaffaa',
    ...
    if (existingSave) {
      const continueBtn = this.add.text(width / 2, btnY + 68, `이어하기  (${existingSave.floor}층)`, {
        fontFamily: '"Neo둥근모", monospace', fontSize: '32px', color: '#aaaaff',
