...
export class EventScene extends Phaser.Scene {
  ...
  private buildEventView(
    ...
    this.add.text(width / 2, 110, event.title, {
      fontFamily: '"Neo둥근모", monospace', fontSize: '30px', color: '#ffddaa', fontStyle: 'bold',
    }).setOrigin(0.5)
    ...
    this.add.text(width / 2, 175, event.flavor, {
      fontFamily: '"Neo둥근모", monospace', fontSize: '17px', color: '#cccccc',
    ...
    event.choices.forEach((choice, i) => {
      ...
      const btn = this.add.text(width / 2, btnStartY + i * 90, choice.label, {
        fontFamily: '"Neo둥근모", monospace', fontSize: '19px',
      ...
      this.add.text(width / 2, btnStartY + i * 90 + 30, choice.detail, {
        fontFamily: '"Neo둥근모", monospace', fontSize: '11px', color: '#556677',
      }).setOrigin(0.5, 0)
    })
  }
  ...
  private showResult(message: string): void {
    ...
    const continueBtn = this.add.text(width / 2, height / 2 + 90, '계속하기', {
      fontFamily: '"Neo둥근모", monospace', fontSize: '20px', color: '#aaaaaa',
