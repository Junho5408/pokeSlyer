...
export class EventScene extends Phaser.Scene {
  ...
  private buildEventView(
    ...
    event.choices.forEach((choice, i) => {
      ...
      const btn = this.add
        .text(width / 2, btnStartY + i * 90, choice.label, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "24px",
      ...
      if (!disabled) {
        ...
        btn.on("pointerover", () =>
          btn.setStyle({ color: "#ffddaa", backgroundColor: "#252545" }),
        );
        btn.on("pointerout", () =>
          btn.setStyle({ color: "#eeeeee", backgroundColor: "#1a1a2e" }),
        );
        btn.on("pointerdown", () => {
          // 다른 버튼 비활성화
      ...
      // 설명
      this.add
        .text(width / 2, btnStartY + i * 90 + 30, choice.detail, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "14px",
          color: "#556677",
        })
  ...
  private showResult(message: string): void {
    ...
    // 결과 패널
    this.add
      .rectangle(width / 2, height / 2 - 30, width - 200, 160, 0x111122)
    ...
    continueBtn.on("pointerout", () =>
      continueBtn.setStyle({ color: "#aaaaaa" }),
    );
