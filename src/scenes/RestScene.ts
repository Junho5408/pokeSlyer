...
export class RestScene extends Phaser.Scene {
  ...
  private buildMainView(run: RunState): void {
    ...
    this.mainView.add(
      this.add
        .text(width / 2, 118, `HP: ${run.hp} / ${run.maxHp}`, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "18px",
          color: "#ff7777",
        })
    ...
    const restBtn = this.add
      .text(width / 2, height - 230, `쉬어가기  (+${healAmt} HP)`, {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "22px",
    ...
    if (canHeal) {
      ...
      this.mainView.add(
        this.add
          .text(width / 2, height - 196, "(이미 최대 HP)", {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "11px",
            color: "#555555",
          })
    ...
    const upgradeBtn = this.add
      .text(width / 2, height - 150, "단련  (카드 강화)", {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "22px",
    ...
    if (hasUpgradable) {
      ...
      upgradeBtn.on("pointerover", () =>
        upgradeBtn.setStyle({ color: "#ffffff" }),
      );
      upgradeBtn.on("pointerout", () =>
        upgradeBtn.setStyle({ color: "#88aaff" }),
      );
      ...
      this.mainView.add(
        this.add
          .text(width / 2, height - 116, "(강화 가능한 카드 없음)", {
            fontFamily: '"Neo둥근모", monospace',
            fontSize: "11px",
            color: "#555555",
          })
  ...
  private buildUpgradeView(run: RunState): void {
    ...
    this.upgradeView.add(
      this.add
        .text(width / 2, 102, "강화된 효과가 미리 표시됩니다", {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: "12px",
          color: "#555577",
        })
