...
export class MapScene extends Phaser.Scene {
  ...
  private buildHUD(run: RunState): void {
    ...
    this.add.text(width / 2, 18, `층  ${run.floor} / 15`, {
      fontFamily: '"Neo둥근모", monospace', fontSize: '22px', color: '#9999cc', fontStyle: 'bold',
    }).setOrigin(0.5, 0)
    ...
    this.add.text(16 + hpBarW / 2, 14, `HP  ${run.hp} / ${run.maxHp}`, {
      fontFamily: '"Neo둥근모", monospace', fontSize: '15px', color: '#ffaaaa',
    }).setOrigin(0.5, 0)

    // 골드 (HP 바 오른쪽)
    this.add.text(272, 18, `💰 ${run.gold} G`, {
      fontFamily: '"Neo둥근모", monospace', fontSize: '20px', color: '#ffcc44',
    }).setOrigin(0, 0)
    ...
    if (relicNames) {
      this.add.text(width / 2, 48, `유물: ${relicNames}`, {
        fontFamily: '"Neo둥근모", monospace', fontSize: '16px', color: '#8888bb',
      }).setOrigin(0.5, 0)
    }
  }
  ...
  private buildNodeView(node: MapNode, run: RunState, pickEnemy: () => string): void {
    ...
    this.add.text(x, dy, style.icon, {
      fontFamily: '"Neo둥근모", monospace', fontSize: `${isBoss ? 16 : 13}px`, color: iconColor,
    }).setOrigin(0.5)

    // 노드 라벨 (아래)
    this.add.text(x, dy + radius + 5, NODE_LABELS[node.type] ?? node.type, {
      fontFamily: '"Neo둥근모", monospace', fontSize: '12px', color: isAccessible ? '#ddddff' : (isVisited ? '#334455' : '#556677'),
    }).setOrigin(0.5, 0)
