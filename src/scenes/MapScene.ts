...
const NODE_STYLE: Record<
  string,
  { color: number; icon: string; glowColor: number }
> = {
...
export class MapScene extends Phaser.Scene {
  ...
  private buildHUD(run: RunState): void {
    ...
    // 층수 (중앙)
    this.add
      .text(width / 2, 18, `층  ${run.floor} / 15`, {
  ...
  private buildNodeView(
    ...
    this.add
      .text(x, dy, style.icon, {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: `${isBoss ? 16 : 13}px`,
        color: iconColor,
      })
      .setOrigin(0.5);
    ...
    // 노드 라벨 (아래)
    this.add
      .text(x, dy + radius + 5, NODE_LABELS[node.type] ?? node.type, {
        fontFamily: '"Neo둥근모", monospace',
        fontSize: "12px",
        color: isAccessible ? "#ddddff" : isVisited ? "#334455" : "#556677",
      })
      .setOrigin(0.5, 0);
    ...
    if (isAccessible) {
      const hitZone = this.add
        .zone(x, dy, (radius + 4) * 2, (radius + 4) * 2)
