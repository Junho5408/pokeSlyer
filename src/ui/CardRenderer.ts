...
const BASE_COLORS: Record<
  string,
  { dark: number; mid: number; accent: number }
> = {
...
export function buildCard(
  ...
  if (hasImage) {
    ...
    c.add(
      scene.add
        .text(costX, costY, String(def.cost), {
    ...
    c.add(
      scene.add
        .text(0, nameY, displayName, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: `${Math.max(11, Math.round(w * 0.1))}px`,
    ...
    c.add(
      scene.add
        .text(0, descY, desc, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: `${Math.max(10, Math.round(w * 0.09))}px`,
          color: "#2a1500",
          wordWrap: { width: w * 0.68 },
          align: "center",
        })
    ...
    c.add(
      scene.add
        .text(gemX, gemY, String(def.cost), {
          fontFamily: '"Neo둥근모", monospace',
    ...
    c.add(
      scene.add
        .text(left + w - 5, top + 5, TYPE_LABELS[def.type] ?? def.type, {
          fontFamily: '"Neo둥근모", monospace',
    ...
    c.add(
      scene.add
        .text(0, contentTop + Math.round(h * 0.13), desc, {
          fontFamily: '"Neo둥근모", monospace',
          fontSize: `${Math.max(8, Math.round(w * 0.075))}px`,
          color: "#ccccdd",
          wordWrap: { width: w - 16 },
  ...
  if (opts.showPrice !== undefined) {
    c.add(
      scene.add
        .text(0, bot + Math.round(h * 0.1), `${opts.showPrice}G`, {
          fontFamily: '"Neo둥근모", monospace',
